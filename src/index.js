...
((doc, docEl) => {
  ...
  const favIconEl = doc.querySelector('link[rel="shortcut icon"]');
  ...
  const checkContrast = () => {
    ...
    ['text-bg', 'link-bg', 'link-text', 'ui-bg'].forEach((badgeName) => {
      ...
      doc
        .querySelectorAll(`[data-validate="${badgeName}"]`)
        .forEach((badge) => {
  ...
  const handleColorPicker = (e) => {
    ...
    if (color) {
      ...
      doc.querySelector(`.inputs [type="text"][data-color="${what}"]`).value =
        hslToHex(color.hsl);
      checkContrast();
    }
  };
  const handleTextColorInput = (e) => {
    ...
    if (color) {
      docEl.style.setProperty(`--sample-${what}`, color.toString());
      doc.querySelector(`.inputs [type="color"][data-color="${what}"]`).value =
        hslToHex(color.hsl);
      checkContrast();
    }
  };
